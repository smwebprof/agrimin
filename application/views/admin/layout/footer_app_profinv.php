...
<script>
       ...
       var commodity_arr = [];
        ...
        $('#cargo_group').change(function(){ 
            ...
            if(cargo_group != '')
             {
               $.ajax({
               'url' : '<?php echo BASE_PATH; ?>Addproformainvoice/fetch_cargo',
               'type': 'post',
             ...
             {
               $.ajax({
               'url' : '<?php echo BASE_PATH; ?>Addproformainvoice/fetch_packing',
               'type': 'post',
               'data' : { cargo_group : cargo_group},
               'success' : function(data)
               {
             ...
             {
               $.ajax({
               'url' : '<?php echo BASE_PATH; ?>Addproformainvoice/fetch_unit',
               'type': 'post',
               'data' : { cargo_group : cargo_group},
               'success' : function(data)
               {
        ...
        function cargo_rows() { 
            var inv_tbody1 = $('#invoice_cargo_item_details').children('tbody');
            //Then if no tbody just select your table 
            var inv_table1 = inv_tbody1.length ? inv_tbody1 : $('#invoice_cargo_item_details');
        ...
        function cargo_amount2(el) {
            ...
            qty = inputs.eq(0).val(),
            rate = inputs.eq(5).val();
            //alert(rate);
            inputs.eq(6).val((qty * rate).toFixed(2));
      ...
      /*$('#div_invoice_items_btn').click(function(){          
        ...
        $('#clients_name').change(function(){
            var comp_id = $('#clients_name').val();
            //alert(comp_id);
             if(comp_id != '')
             {
               $.ajax({
               'url' : '<?php echo BASE_PATH; ?>Addproformainvoice/fetch_clientdata',
               'type': 'post',
               'data' : { id : comp_id},
               'success' : function(data)
               {
                 res = data.split("|");
                 //alert(res);
                 $('#client_address').html(res[0]);
                 $('#postal_code').val(res[1]);
                 $('#company_country').val(res[4]);
                 $('#company_state').val(res[5]);
                 $('#company_city').val(res[6]);
                 //$('#office_no').val(res[1]);
