...
class Addinvoicepaymentregister extends CI_Controller {
	...
	public function index()
	{
        ...
        $result = $this->Invoice_master->getInvoiceById($id);
        //print_r($result);exit;
        if ($_POST) {
        	//print_r($_POST);exit;
        	...
        	$file_no = $_POST['invoice_file_no'];

        	$resultdata = $this->Invoice_master->addPayementInvoiceData($this->input->post());
        	#$resultdata = 1;
        	if ($resultdata) {

        		$UpdateInvoiceBalanceData = $this->Invoice_master->UpdateInvoiceBalance($this->input->post());

                $invoice_recd_amt = (float)$_POST['invoice_recd_amt'] + (float)$_POST['invoice_rec_amt'];
        		if ($_POST['invoice_amt']==$invoice_recd_amt) { 
	        	...
	        	/*$this->data['success'] = "Data saved successfully!!!. Payment Id is: ".$resultdata;

	        	$this->data['title'] = 'ACI - Invoicefileregister';
				$this->data['invoice_data'] = @$result;
				$this->data['layout_body']='addinvoicepaymentregister';
