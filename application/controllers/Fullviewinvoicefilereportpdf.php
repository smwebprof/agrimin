       ...
       #$logo = ASSETS_PATH.'img/agrilogo269x74.jpg';
        ...
        $pdf9 = '<table width="100%" cellpadding="0" border="0">
          ...
          foreach($data as $company_address){  
            if ($result[0]['file_id']<102 && $branch_details['sortname']=='SG') {  
              $pdf9 .= '<td width="50%">
                    <table width="100%" border="0">
                        <tr><td></td></tr>
                        <tr><td></td></tr>
        ...
        foreach($rows as $invoice_data){
          ...
          /*$pdf4 .= '<table border="1" cellspacing="0" cellpadding="5">
            ...
            <tr>
              ...
              if (!empty(@$branch_details['vat_no'])) { $pdf6 .= '<tr><td>VAT No : </td><td>'.@$branch_details['vat_no'].'</td></tr>'; }

              if (!empty(@$branch_details['bank_cleaing_no'])) {  $pdf6 .= '<tr><td>Clearing No : </td><td>'.@$branch_details['bank_cleaing_no'].'</td></tr>'; }

                      $pdf6 .= '<tr><td>Reference : </td><td>'.@$result[0]['file_no'].'</td></tr>';                    
